...
rast = rasteriser.rasteriser(1920, 1080)
...
with open("text.bmp", "wb") as test:
    ...
    for byte in int(1920).to_bytes(4,'big'): string.append(byte)
    for byte in int(1080).to_bytes(4,'big'): string.append(byte)
    for byte in bytearray.fromhex('0100180000000000'): string.append(byte)
    for byte in int(1920*1080*4).to_bytes(4,'big'): string.append(byte)
    ...
    for x in rast.image_pixels:
        for y in x:
            count += 1
            for byte in bytearray.fromhex(y[1:]): test.write(byte.to_bytes())
            if count == 2:
                count = 0
                for byte in bytearray.fromhex('0000'): test.write(byte.to_bytes())