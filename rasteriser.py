class rasteriser:
    ...
    def bottom_triangle(self, triangle, colour):
        ...
        for y in range(v1[1], topv[1]):
            for x in range(int(x1), int(x2)):
                if x > 0 and x < self.width:
                    self.image_pixels[x][y] = colour
            x1 += islope1
            x2 += islope2
    ...
    def draw_triangle(self, triangle, colour):
        if (triangle[1] == triangle[3] and triangle[5] < triangle[3]) or (triangle[3] == triangle[5] and triangle[1] < triangle[5]) or (triangle[1] == triangle[5] and triangle[3] < triangle[5]):
            ...
            if new_tri[0][1] != new_tri[1][1] and new_tri[1][1] != new_tri[2][1] and new_tri[2][1] and new_tri[0][1]:
                newx = new_tri[0][0] + (new_tri[1][1] - new_tri[0][1])/(new_tri[2][1] - new_tri[0][1]) * (new_tri[2][0] - new_tri[1][1])
